<%style%>
<%if $head.dir=='ltr'%> 
  @import url('/widget/header/userbar-btn-header/ltr/ltr.css?__inline');
<%else%> 
  @import url('/widget/header/userbar-btn-header/rtl/rtl.css?__inline');
<%/if%>
<%/style%>

<%*   声明对ltr/rtl的css依赖    *%>
<%if $head.dir=='ltr'%>
  <%require name="buy:widget/header/userbar-btn-header/ltr/ltr.more.css"%> 
<%else%> 
  <%require name="buy:widget/header/userbar-btn-header/rtl/rtl.more.css"%> 
<%/if%>
	...
	<%script%>
		...
		require.async(["buy:widget/header/userbar-btn-header/userbar-btn-header-async.js"],function(init){
			init();
		});
	<%/script%>
