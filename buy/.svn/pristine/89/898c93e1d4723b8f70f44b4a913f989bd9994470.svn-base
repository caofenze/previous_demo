...
var $ = require("buy:widget/ui/jquery/jquery.js"),
	helper = require("buy:widget/ui/helper/helper.js"),
	date = require("buy:widget/ui/date/date.js"),
	UT = require('buy:widget/ui/ut/ut.js');

var isTested = true;//($.trim(conf.weather.isHeaderTest) === ""? false : true);

var iconGroup = [];

var TPL_CONF = {
	tpl: '<li class="#{className}"><img src="#{' + (isTested? 'iconBig': 'iconSmall') + '}" title="#{descrip}"><div class="weather-info"><p class="text">#{day}</p><span class="temperature_p">#{low}#{hi}℃</span><span title="#{humidity}" style="display: inline-block;"><i class="dot_img"></i><span class="temperature_hum">#{avehumidity}%</span></span></div></li>',
	tplMoreDate: '<li class="#{className}"><p class="wea-fl"><span>#{week}</span><span class="wea-small">#{date}</span></p><img src="#{iconBig}" title="#{descrip}"><p class="wea-fr"><span class="temperature_big">#{low}#{hi}℃</span><span class="wea-hide" title="#{humidity}"><i class="dot_img_head"></i><span class="humidity">#{avehumidity}%</span></span></p></li>',
	tplMoreDay: '<li class="#{className}"><p>#{day}</p><img src="#{iconDefault}" title="#{descrip}"><p><span class="temperature_big">#{low}#{hi}℃</span>' + (isTested? '<br /><span class="weather-more_tip-remind">#{descrip}</span>': '') + '<span title="#{humidity}" class="wea-flr"><i class="dot_img_head"></i><span class="humidity">#{avehumidity}%</span></span></p></li>',
...
Gl.weather = {
	...
	init: function(weatherCityDir) {
		var that = this,
			//default city
			cityId = that.cityId = ($.cookie("weatherCity") || conf.weather.defaultCity),
			//weatherCityDir = "/static/widget/common/header/weather/" + conf.weather.country + "/weather_city.js",
			isIE6 = /MSIE 6.0/.test(navigator.userAgent);

		conf.weather.bigIconPath = isIE6 ? conf.weather.bigIconPath + "_png8/" : conf.weather.bigIconPath + "/";
		conf.weather.smallIconPath = isIE6 ? conf.weather.smallIconPath + "_png8/" : conf.weather.smallIconPath + "/";

		/*$.ajax({
			url: "/static/widget/common/header/weather/" + conf.weather.country + "/weather_city.js?20130117",
			dataType: "script",
			cache: true,
			success: function() {
				//(need the cites's list)
				that.bindEvent(that);
				that.requestData();
			}
		});*/
	...
	bindEvent: function(that) {
		var moreWrap         = $("#weatherMoreWrap"),
			...
			thi         = this,
			tipsClose   = $("#tipsClose"),
			cityListDir = "buy:widget/ui/weather/" + conf.country + "/" + conf.country + "_flat.js",
		...
		tipsClose.live("mouseover", function() {
			...
			$.cookie("weatherTipClose", $.cookie("weatherCity") || conf.weather.defaultCity, {expires: date});
	...
	fixData: function(data) {
        ...
        /*
	// 换肤时更新天气icon
	refreshIcon: function(themeId) {
		var iconPath = conf.weather.testBigIcon;
		if (iconPath == "") {
			return;
		}
		iconPath += "#{themeId}/#{icon}.png";
		if (iconGroup && (iconGroup.length == conf.weather.dataLength)) {
			if ($("#weatherMore").length > 0) {
				$("#weatherMore").find("li").each(function(index, ele) {
					$(ele).children('img').attr("src", helper.replaceTpl(iconPath, {
						themeId: themeId,
	...
	formatData: function(data, n) {
		...
		if(isTested) {
			...
			/*if(conf.pageType == "index") {
				iconBigPath = _conf.testBigIcon + ( $.cookie("hao123themename") || "1") + "/";
			} else {
		...
		/*return {
			hi: data.hi === "N/A" ? data.hi + " " : data.hi + "℃",
			low: data.low === "N/A" ? data.low + " " : data.low + "℃",
			icon: _conf.iconPath + data.part[0].icon + ".png",	//only show daytime's icon
			date: data["@attributes"].dt.split(" ")[1],
			week: _conf.week[data["@attributes"].t],
			day: _conf.day[data["@attributes"].d],
			title: _conf.tips.click,
			url: "http://www.weather.com/weather/"+ _conf.dayUrl[data["@attributes"].d] +"/" + this.cityId,
			className: ""	//add a new key about className and set the default value
		};*/
	},
	...
	render: function(data) {
		...
		$.each(data, function(key, val) {
			...
			if ( dateSub === 0 ) {
				...
				tempMore += helper.replaceTpl(tplMoreDay, val);
			} else {
				...
				tempMore += helper.replaceTpl(tplMoreDate, val);
			}
		...
		if(conf.weather.remindSwitch === 'on' && ( ($.cookie("weatherTipClose") !== $.cookie("weatherCity")) || !$.cookie("weatherTipClose")) && weatherRemind) {
