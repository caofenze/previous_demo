...
<%widget name="buy:widget/header/weather/`$head.dir`/`$head.dir`.tpl"%>
<%script%>
	//conf.weather
	conf.weather = {
		prefixUrl: "<%$body.weather.prefixUrl%>",
		country: "<%$country%>",
		defaultCity: "<%$body.weather.defaultCity%>",
...
<%/script%>
<%widget name="buy:widget/header/weather/`$sysInfo.country`/`$sysInfo.country`.tpl" country=$sysInfo.country%>
