...
require("buy:widget/ui/jquery/widget/jquery.sethome/jquery.sethome.js");
...
Gl.logo = function (type) {
	...
	indexLogo.on("click", function ( e ) {
		...
		if(logoTip && isNotEmpty(conf.logo.autoCloseTip)) {
			logoTip.hide();
			$.cookie("logoTipsHide", conf.logoTips.country || "jp");
		}
		if( type === "index" && conf.logo.notOpenNew === "true" ){
			...
			var url = $(this).attr("href"),
				...
				paramVal;
			if(paramKey.length && (paramVal = helper.getQuery(location.href)[paramKey])){
				url += (url.indexOf("?") != -1?"&":"?") + paramKey + "=" + encodeURIComponent(paramVal);
			}
...
Gl.logoTips.prototype = {
	...
	init: function () {
		...
		if (myConf.show) {
			...
			if (myConf.alwaysShow) {
				...
				(!$.cookie("logoTipsHide") || $.cookie("logoTipsHide") !== that.conf.country) && that.show();
			} else {
				that.setCookie("oldFriend", myConf.version, function() {
					that.show();
				});
			}
		}
	...
	bindEvent: function () {
		...
		that.tipLink.on("click", function (e) {
			...
			if(confAct.isHidden === "0" && actTips) {
				confAct.logoTips.userOption && $.cookie("logoTipsHide",that.conf.country);
			} else {
				that.conf.userOption && $.cookie("logoTipsHide",that.conf.country);
			}
			that.tip.hide();
			UT.send({
		...
		that.closeBtn.on("click", function (e) {
			...
			$.cookie("logoTipsHide",that.conf.country);
		});
	},
	show: function () {
		var that = this,
	...
	setCookie: function (key, value, callback, expires) {
		var that = this;
		$.cookie(key) !== value && callback();
		$.cookie(key, value, {expires: expires || 2000});
	},
