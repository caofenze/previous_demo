...
<%*   声明对ltr/rtl的css依赖    *%>
<%if $head.dir=='ltr'%> <%require name="buy:widget/header/logo/ltr/ltr.more.css"%> <%else%> <%require name="buy:widget/header/logo/rtl/rtl.more.css"%> <%/if%>
...
<div class="userbar-logo <%if $head.dir=='ltr'%>fl<%else%>fr<%/if%>" log-mod="logo">
	...
	<span class="userbar-logo_slogan"<%if !empty($body.logo.sloganSize)%> style="font-size:<%$body.logo.sloganSize%>"<%/if%>><%$body.logo.slogan%></span>
...
<%script%>
	...
	require.async(["buy:widget/ui/jquery/jquery.js", "buy:widget/header/logo/logo-async.js"], function ($) {
