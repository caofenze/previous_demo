...
Gl.clock = function(el) {
    ...
    if (_conf.headerTest && _conf.openCal === "1") {
        ...
        $glo.one("o.onceload", function() {
            require.async("buy:widget/header/clock/calendar-async.js", function(init) {
                var wk = $.extend(true, [], conf.date.lunar && conf.date.lunar.wk);
                init($glo, _conf.moreBtn, url, _co, _co.weeks || wk);
            });
        });
        // add UT
        ...
        if (_con && _con.isHidden === "0" && _con.time) { // countdown
